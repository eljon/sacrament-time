...
(function () {
  ...
  var _actx = null, _lastTick = -1, _unlocked = false, _keepAlive = null;
  ...
  // A permanent silent source keeps the context "running" so iOS can't re-suspend it
  // between ticks. That re-suspension is why a drag seemed not to unlock: resume()
  // ran the context for an instant, iOS suspended it again before the next tick, and
  // every tick then found it suspended. With this running, ONE successful resume
  // (from any gesture, including a drag) sticks and all later ticks sound. Pure Web
  // Audio — no <audio> element anywhere, so nothing can stall the UI.
  function keepAlive(ctx) {
    if (_keepAlive) return;
    try {
      var o = ctx.createOscillator(), g = ctx.createGain();
      g.gain.value = 0; o.frequency.value = 30;                 // silent, just keeps the graph active
      o.connect(g); g.connect(ctx.destination);
      o.start(0); _keepAlive = o;                               // never stopped
    } catch (e) {}
  }
  function unlockTick() {
