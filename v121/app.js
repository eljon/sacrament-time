...
(function () {
  ...
  // ---- Tick sound -----------------------------------------------------------
  // A soft, short Web Audio click on every coverflow card-snap and picker notch.
  // Web Audio (NOT <audio>): createOscillator/start is cheap and runs off the main
  // thread, so rapid ticks during a fast flick never stall scrolling — an
  // <audio>.play() per snap froze the UI on iOS. Fixed pitch (no frequency sweep)
  // so it reads as a clean "tock", not a chirp. Unlocked with a silent buffer
  // inside the first touch gesture.
  var _actx = null, _lastTick = -1, _unlocked = false, _silent = null;
  // A tiny SILENT <audio> clip. On iOS a drag/pan gesture often won't grant a bare
  // AudioContext.resume() the activation it needs (only a discrete tap does — which
  // is why the tick was silent until you tapped + / edit), but an <audio>.play()
  // DOES activate the audio session from a drag. So we play this silent clip to
  // light up the session, then resume Web Audio in the same gesture — after which
  // Web Audio runs and drives the actual ticks (no <audio> per tick → no freeze).
  ...
  function silentEl() {
    ...
    try { _silent = new Audio(SILENT_WAV); _silent.preload = "auto"; } catch (e) { _silent = null; }
    return _silent;
  }
  ...
  function audioCtx() {
    ...
    if (!AC) return null;
    try { _actx = new AC(); setAudioSession(); } catch (e) { _actx = null; }
    return _actx;
  }
  function unlockTick() {
    var ctx = audioCtx(); if (!ctx) return;
    setAudioSession();
    if (ctx.state === "running") { _unlocked = true; return; }   // already live — nothing to do
    // Not running yet: activate the iOS audio session via a silent <audio> play
    // (works on drag gestures), then resume + prime Web Audio in the same gesture.
    var el = silentEl();
    if (el) { try { el.currentTime = 0; var pr = el.play(); if (pr && pr.catch) pr.catch(function () {}); } catch (e) {} }
    try { ctx.resume(); } catch (e) {}
    if (!_unlocked) {
      try {
        var b = ctx.createBuffer(1, 1, 22050), s = ctx.createBufferSource();
        s.buffer = b; s.connect(ctx.destination); s.start ? s.start(0) : s.noteOn(0);
        _unlocked = true;
      } catch (e) {}
    }
  }
  function tick() {
    ...
    if (t - _lastTick < 0.03) return;                        // one tick per ~30ms, no pile-up
    _lastTick = t;
    try {
      var osc = ctx.createOscillator(), g = ctx.createGain();
      osc.type = "sine";
      osc.frequency.value = 900;                             // fixed pitch → no sweep → no "chirp"
      g.gain.setValueAtTime(0.0001, t);
      g.gain.exponentialRampToValueAtTime(0.22, t + 0.004);  // soft attack
      g.gain.exponentialRampToValueAtTime(0.0001, t + 0.028);// quick clean decay → a soft "tock"
  ...
  function init() {
    initHaptics();
    // Create the AudioContext up front (suspended), so the FIRST interaction only
    // has to resume() an already-established context. Creating AND resuming a
    // brand-new context in the same gesture tick doesn't reliably unlock on iOS —
    // that's why the tick was silent on a first coverflow drag but worked after a
    // tap on + (by then the context already existed).
    audioCtx();
    // Unlock on the first gesture, and warm the Taptic engine once — silently, so
    // a stray first tap doesn't tick. Bind many gesture types since iOS is picky
    // about which one "activates" audio (a drag's pointerdown vs a tap's click).
