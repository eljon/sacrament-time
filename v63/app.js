...
(function () {
  ...
  // successive cards shrinks going outward (50, 30, 22, 16, 11, 7) so far cards
  // pack tighter instead of drifting off on a constant step. Interpolated so
  // dragging stays smooth.
  var CF_X = [0, 50, 80, 102, 118, 129, 136];
  function cardX(ao) {
  ...
  function applyCard(el, o, isCenter, dateStr) {
    ...
    var x = s * cardX(ao);
    // tilt ramps progressively: 0° at centre → 50° at the first neighbour → 90°
    // (edge-on) at the farthest card
    var deg = ao <= 1 ? 50 * ao : 50 + 40 * Math.min(1, (ao - 1) / (CF_WINDOW - 1));
