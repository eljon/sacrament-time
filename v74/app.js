...
(function () {
  ...
  function refreshFlyAnchors() {
    ...
    f.groups.forEach(function (g) {
      ...
      // the source card's Cover Flow rotation, so each clone STARTS rotated exactly
      // like the calendar it leaves, then unrotates to face-on as it lands in the graph
      var cm = new DOMMatrix(getComputedStyle(g.card).transform);
      g.srcRY = Math.atan2(cm.m13, cm.m11) * 180 / Math.PI;
  ...
  function updateFly() {
    ...
    f.groups.forEach(function (g) {
      ...
      if (!active) return;
      // The card's Cover Flow rotation decays to 0 across the flight, so a clone
      // leaves looking exactly like its (rotated) calendar tile and arrives face-on.
      var ry = g.srcRY * (1 - eP);
      ...
      // Positions are DOCUMENT coordinates (the layer is absolute, so it scrolls).
      var bx = g.sBar.cx + (g.tBar.cx - g.sBar.cx) * eP;
      var by = g.sBar.cy + (g.tBar.cy - g.sBar.cy) * eP;
      ...
      g.barEl.style.transform = "perspective(760px) translate3d(" + bx + "px," + by + "px,0) rotateY(" + ry +
        "deg) translate(-50%,-50%) rotate(" + deg + "deg) scale(" + (tw / g.barW0) + "," + (th / g.barH0) + ")";
      // month & day clones: glide from the card into the axis, scaling to fit
      placeClone(g.monEl, g.sMon, g.tMon, g.monScale0 + (g.monScale1 - g.monScale0) * eP, eP, ry);
      placeClone(g.dayEl, g.sDay, g.tDay, g.dayScale0 + (g.dayScale1 - g.dayScale0) * eP, eP, ry);
    });
  }
  function placeClone(el, s, t, scale, eP, ry) {
    var x = s.cx + (t.cx - s.cx) * eP, y = s.cy + (t.cy - s.cy) * eP;   // document coords
    el.style.transform = "perspective(760px) translate3d(" + x + "px," + y + "px,0) rotateY(" + ry +
      "deg) translate(-50%,-50%) scale(" + scale + ")";
  }
