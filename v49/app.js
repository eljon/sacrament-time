...
(function () {
  ...
  function renderHistoryGraph() {
    ...
    // first paint = final positions; the fly-in only runs once, on reveal
    if (state.hgShown || prefersReduced()) return;
    maybeRevealGraph();
  }
  // The first time the graph scrolls into view, fly the real calendar bars and
  // dates from the picker into their graph slots (a per-element FLIP).
  function maybeRevealGraph() {
    var section = $("historyGraph");
    if (!section || section.hidden || state.hgShown || prefersReduced()) return;
    var r = section.getBoundingClientRect();
    var vh = window.innerHeight || document.documentElement.clientHeight;
    if (r.top >= vh * 0.9 || r.bottom <= 0) return;   // not in view yet
    state.hgShown = true;
    var track = $("cfTrack");
    if (track) { track.classList.add("cf-shed"); setTimeout(function () { track.classList.remove("cf-shed"); }, 1400); }
    state.hgItems.forEach(function (it, i) {
      var card = state.cardEls && state.cardEls[it.date];
      flyIntoPlace(it.bar, card && (card.querySelector(".cal-bar") || card), i, true);
      flyIntoPlace(it.xc, card, i, false);
    });
  }
  // Move `el` so it starts at `srcEl`'s on-screen position (as a small bar for
  // the graph bars) and glides to its real slot; falls back to a drop-in.
  function flyIntoPlace(el, srcEl, i, isBar) {
    var dst = el.getBoundingClientRect();
    var tf;
    if (srcEl) {
      var src = srcEl.getBoundingClientRect();
      var dx = (src.left + src.width / 2) - (dst.left + dst.width / 2);
      var dy = (src.top + src.height / 2) - (dst.top + dst.height / 2);
      tf = "translate(" + dx.toFixed(1) + "px," + dy.toFixed(1) + "px)" + (isBar ? " rotate(90deg) scaleY(.12)" : " scale(.85)");
    } else {
      tf = "translateY(-160px)" + (isBar ? " rotate(90deg) scaleY(.12)" : "");
    }
    ...
    el.style.opacity = "0";
    void el.offsetWidth;                                  // commit the start
    var delay = (i * 60) + "ms";
    el.style.transition = "transform .8s cubic-bezier(.2,.72,.24,1) " + delay + ", opacity .45s ease " + delay;
    el.style.transform = "none";
    el.style.opacity = "1";
    var clear = function () {
      el.classList.remove("flying");
      el.style.transition = ""; el.style.transform = ""; el.style.opacity = ""; el.style.transformOrigin = "";
