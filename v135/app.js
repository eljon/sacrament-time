...
(function () {
  ...
  function cfSyncCenter() {
    ...
    // Rebuilding the whole hero (timeline, clock hands, gradients) on every crossing
    // hitches a fast flick. While the picker is moving, defer it and flush ONCE when
    // it settles; the cards themselves show which week is centred meanwhile.
    if (state.cf.dragging || state.cf.raf != null) state.heroDirty = true;
    else renderHeroBody();
  }
