...
(function () {
  ...
  function updateFly() {
    ...
    f.groups.forEach(function (g) {
      ...
      // visibility (not display) when idle — flipping it doesn't reflow, so the
      // fly-start frame stays cheap; only written when the active state changes.
      if (g._active !== active) {
        g._active = active;
        var v = active ? "visible" : "hidden";
        g.day.el.style.visibility = v; g.mon.el.style.visibility = v; g.bar.el.style.visibility = v;
