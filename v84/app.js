...
(function () {
  ...
  function refreshFlyAnchors() {
    ...
    f.end = $("historyGraph").getBoundingClientRect().top + sy - vh * 0.12;    // finish when the graph has settled near the top, not mid-viewport
    ...
    f.groups.forEach(function (g) {
      var tDay = docBox(g.it.xc.querySelector(".hg-day")), tMon = docBox(g.it.xc.querySelector(".hg-mon")), tBar = docBox(g.it.bar);
      // source centre = the real calendar piece (document coords); target centre =
      // the graph piece. The clone flies its centre from source → target in flat
      // screen space, so the perspective never distorts the travel or the size.
      prepPiece(g.day, docBox(g.calDay), tDay, g.day.natH ? tDay.h / g.day.natH : 1);
      prepPiece(g.mon, docBox(g.calTop), tMon, g.mon.natH ? tMon.h / g.mon.natH : 1);
