...
(function () {
  ...
  function init() {
    initHaptics();
    // prime the Taptic engine on the very first touch (strong user activation),
    // so later in-drag ticks aren't swallowed as the first-ever haptic
    window.addEventListener("pointerdown", function primeHaptic() { haptic(); }, { once: true });
    loadStops();
    ...
    // deceleration, when scroll events arrive too coarsely to look smooth. The loop
    // parks itself once the position holds still, and any scroll event re-kicks it.
    var flyRaf = null, flyLastY = -1, flyStill = 0;
    function flyLoop() {
      var y = window.pageYOffset;
      if (y === flyLastY) { if (++flyStill > 10) { flyRaf = null; return; } }
      else { flyStill = 0; flyLastY = y; }
      updateFly();
      flyRaf = requestAnimationFrame(flyLoop);
    }
    function flyKick() { if (flyRaf == null) { flyStill = 0; flyLastY = -1; flyRaf = requestAnimationFrame(flyLoop); } }
    window.addEventListener("scroll", flyKick, { passive: true });
