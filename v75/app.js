...
(function () {
  ...
  function updateFly() {
    ...
    f.groups.forEach(function (g) {
      ...
      flyPiece(f, g, g.day, src, ry, z, eP, null);
      flyPiece(f, g, g.mon, src, ry, z, eP, null);
      flyPiece(f, g, g.bar, src, ry, z, eP, eP);   // bar also rolls 90° + stretches
    });
  }
  function flyPiece(f, g, pc, src, ry, z, eP, barMorph) {
    var sc = src.scale + (pc.dstScale - src.scale) * eP;
    var tx = src.x + (pc.tx1 - src.x) * eP, ty = pc.ty1 * eP;
    ...
    var M = new DOMMatrix().translateSelf(f.baseX, f.baseY).multiply(f.P)
      .multiply(new DOMMatrix().translateSelf(tx, 0, z).rotateSelf(0, ry, 0).scaleSelf(sc, sc))
