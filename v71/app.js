...
(function () {
  ...
  function setupFly() {
    ...
    layer.innerHTML = "";
    if (state.fly && state.fly.raf != null) cancelAnimationFrame(state.fly.raf);   // stop any in-flight glide
    state.fly = null;
    ...
    state.fly = { groups: groups, anchored: false, start: 0, end: 1, pCur: 0, pMid: 0, pTarget: 0, raf: null };
    refreshFlyAnchors();
    updateFly();
  }
  function docBox(el) {
  ...
  function refreshFlyAnchors() {
    ...
    f.groups.forEach(function (g) {
      ...
      // Fix each clone's intrinsic box ONCE; the morph is pure transform after this.
      // bar base = the calendar bar's rendered size; it scales (and rotates 90°) to
      // the vertical graph bar. Rounded ends come from a fixed radius = half height.
      g.barW0 = g.sBar.w || 1; g.barH0 = g.sBar.h || 1;
      g.barEl.style.width = g.barW0 + "px"; g.barEl.style.height = g.barH0 + "px";
      g.barEl.style.borderRadius = (g.barH0 / 2) + "px";
  ...
  // The raw scroll position only sets a TARGET progress; a rAF spring eases the
  // rendered progress toward it (accelerate, then decelerate), so the fly glides
  // like kinetic scrolling instead of snapping to each discrete scroll event —
  // this is what makes slow, chunky scrolling feel smooth.
  function updateFly() {
    var f = state.fly; if (!f) return;
    var sy = window.pageYOffset;
    var p = (sy - f.start) / (f.end - f.start);
    if (f.maxScroll != null && sy >= f.maxScroll - 1) p = 1;   // can't scroll further → fully assemble
    f.pTarget = p < 0 ? 0 : p > 1 ? 1 : p;
    if (f.pTarget > 0 && !f.anchored) refreshFlyAnchors();
    flyEnsureRaf();
  }
  function flyEnsureRaf() { var f = state.fly; if (f && f.raf == null) f.raf = requestAnimationFrame(flyStep); }
  function flyStep() {
    var f = state.fly; if (!f) return;
    f.raf = null;
    var target = f.pTarget || 0;
    if (f.pCur == null) f.pCur = target;
    if (f.pMid == null) f.pMid = f.pCur;
    // Two cascaded low-pass filters: a step in the scroll target produces an
    // S-curve response — motion accelerates in, glides, and decelerates out —
    // with no overshoot (both poles are real), so bars never bounce past the
    // graph. This is the kinetic/momentum feel, driven every frame by rAF.
    var a = 0.2;
    f.pMid += (target - f.pMid) * a;
    f.pCur += (f.pMid - f.pCur) * a;
    if (f.pCur < 0) f.pCur = 0; else if (f.pCur > 1) f.pCur = 1;
    flyRender(f.pCur);
    if (Math.abs(target - f.pCur) > 0.0003 || Math.abs(target - f.pMid) > 0.0003) {
      f.raf = requestAnimationFrame(flyStep);   // keep gliding until it settles
    } else {
      if (f.pCur !== target) { f.pCur = f.pMid = target; flyRender(target); }
      if (target <= 0) f.anchored = false;      // re-measure next time the fly starts
    }
  }
  function flyRender(p) {
    var f = state.fly; if (!f) return;
    ...
    f.groups.forEach(function (g) {
      ...
      // Only written when the active state actually changes, so there's no
      // per-frame layout churn during the scrub.
      if (g._active !== active) {
        g._active = active;
        var d = active ? "block" : "none";
        g.barEl.style.display = d; g.monEl.style.display = d; g.dayEl.style.display = d;
        g.barEl.style.opacity = vis; g.monEl.style.opacity = vis; g.dayEl.style.opacity = vis;
      }
      if (!active) return;
      // bar: horizontal calendar bar → vertical start→end graph bar, morphed purely
      // by transform (translate + rotate + non-uniform scale) — no layout/paint.
      var bx = g.sBar.cx + (g.tBar.cx - g.sBar.cx) * eP - sx;
      var by = g.sBar.cy + (g.tBar.cy - g.sBar.cy) * eP - syo;
      var tw = g.sBar.w + (g.tBar.h - g.sBar.w) * eP;   // width before the 90° rotation
      var th = g.sBar.h + (14 - g.sBar.h) * eP;          // height before the rotation
      g.barEl.style.transform = "translate3d(" + bx + "px," + by + "px,0) translate(-50%,-50%) rotate(" +
        deg + "deg) scale(" + (tw / g.barW0) + "," + (th / g.barH0) + ")";
  ...
  function applyCard(el, o, isCenter, dateStr) {
    ...
    var transform = "translateX(" + x + "px) translateZ(" + ((1 - cl) * 52) + "px) " +
      "rotateY(" + (-s * deg) + "deg) scale(" + cardScale(ao) + ")";
  ...
  function init() {
    ...
    // drive the history-graph fly: scroll only updates the target progress; the
    // rAF spring in flyStep eases the actual motion, so it glides with momentum.
    window.addEventListener("scroll", updateFly, { passive: true });
