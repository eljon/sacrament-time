...
(function () {
  ...
  var _actx = null, _buf = null, _bridge = null, _master = null, _keepAlive = null, _pool = [], _pi = 0, _warmed = false, _lastTickT = 0;
  ...
  function audioCtx() {
    ...
    try {
      ...
      _master = _actx.createGain(); _master.gain.value = 0.5; _master.connect(_actx.destination);   // reused → no per-tick node alloc
      // Bridge: an <audio> routed THROUGH the context at 0 gain. Playing it (which
      // iOS allows from a DRAG) forces the context to run when resume() alone won't.
      try {
        _bridge = new Audio(TICK_WAV);
        var bsrc = _actx.createMediaElementSource(_bridge);
        var bg = _actx.createGain(); bg.gain.value = 0;
        bsrc.connect(bg); bg.connect(_actx.destination);
  ...
  function initTickPool() {
    setAudioSession();
    audioCtx();                             // create context + decode tick buffer + wire the bridge
    for (var i = 0; i < 3; i++) {           // tiny <audio> fallback (only if Web Audio can't run at all)
      var a = new Audio(TICK_WAV); a.preload = "auto"; try { a.load(); } catch (e) {}
      _pool.push(a);
    }
  }
  // First gesture: resume Web Audio and play the SILENT bridge. Because the bridge
  // is routed through the context, playing it (allowed from a drag) forces the
  // context to "running", after which every tick is a cheap OFF-THREAD buffer play
  // — no <audio> per tick, so no freeze.
  function unlockTick() {
    setAudioSession();
    var ctx = audioCtx();
    ...
    if (_warmed) return;
    _warmed = true;
    if (_bridge) { try { var p = _bridge.play(); if (p && p.catch) p.catch(function () {}); } catch (e) {} }
  }
  function tick() {
    ...
    _lastTickT = now;
    var ctx = _actx;
    if (ctx && ctx.state === "running" && _buf && _master) {   // off-thread buffer play → no freeze
      try { var s = ctx.createBufferSource(); s.buffer = _buf; s.connect(_master); s.start(0); } catch (e) {}
      return;
    }
    if (ctx && ctx.state !== "running") { try { ctx.resume(); } catch (e) {} }
    if (!_pool.length) return;                          // <audio> fallback until Web Audio runs
