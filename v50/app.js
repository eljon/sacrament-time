...
(function () {
  ...
  function renderHistoryGraph() {
    ...
    var recs = sortedRecords().slice(-10);
    ...
    recs.forEach(function (r) {
      ...
      xc.innerHTML = '<span class="hg-d">' + d.getDate() + '</span><span class="hg-m">' + MON[d.getMonth()].toUpperCase() + '</span>';
      xrow.appendChild(xc);
  ...
  function setupFly() {
    ...
    state.hgItems.forEach(function (it) {
      ...
      var dateEl = document.createElement("div"); dateEl.className = "hg-fly-date"; dateEl.innerHTML = it.xc.innerHTML;
      layer.appendChild(dateEl);
      clones.push({ it: it, card: card, calBar: card.querySelector(".cal-bar"), barEl: barEl, dateEl: dateEl });
    });
    if (!clones.length) return;
    state.fly = { clones: clones, anchored: false, start: 0, end: 1 };
    refreshFlyAnchors();
    updateFly();
  }
  function docBox(el) {
    var r = el.getBoundingClientRect();
    return { x: r.left + window.pageXOffset + r.width / 2, y: r.top + window.pageYOffset + r.height / 2, w: r.width, h: r.height };
  }
  function refreshFlyAnchors() {
    ...
    f.clones.forEach(function (c) {
      c.cb = docBox(c.calBar || c.card);       // source: the calendar bar
      c.cc = docBox(c.card);                    // source: the calendar card (for the date)
      c.gb = docBox(c.it.bar);                  // target: the graph bar
      c.gx = docBox(c.it.xc);                   // target: the graph date
    });
    f.anchored = true;
  }
  function updateFly() {
    ...
    var shed = Math.min(1, p / 0.18);           // calendar contents leave early
    var appear = Math.max(0, (p - 0.82) / 0.18);// graph bars arrive late
    var active = p > 0.0015 && p < 0.9985;
    f.clones.forEach(function (c) {
      if (c.calBar) c.calBar.style.opacity = String(1 - shed);
      var day = c.card.querySelector(".cal-day"), top = c.card.querySelector(".cal-top");
      if (day) day.style.opacity = String(1 - shed);
      if (top) top.style.opacity = String(1 - shed);
      c.it.bar.style.opacity = String(appear);
      c.it.xc.style.opacity = String(appear);
      c.barEl.style.opacity = active ? "1" : "0";
      c.dateEl.style.opacity = active ? "1" : "0";
      if (!active || !c.gb) return;
      var bx = c.cb.x + (c.gb.x - c.cb.x) * eP, by = c.cb.y + (c.gb.y - c.cb.y) * eP;
      var w = 37 + (c.gb.h - 37) * eP, h = 4 + (14 - 4) * eP, rot = 90 * eP;
      c.barEl.style.left = bx + "px"; c.barEl.style.top = by + "px";
      c.barEl.style.width = w + "px"; c.barEl.style.height = h + "px";
      c.barEl.style.transform = "translate(-50%,-50%) rotate(" + rot + "deg)";
      var dx = c.cc.x + (c.gx.x - c.cc.x) * eP, dy = c.cc.y + (c.gx.y - c.cc.y) * eP;
      var ds = 1.7 + (1 - 1.7) * eP;
      c.dateEl.style.left = dx + "px"; c.dateEl.style.top = dy + "px";
      c.dateEl.style.transform = "translate(-50%,-50%) scale(" + ds + ")";
    });
  }
  ...
  // the centre card sits biggest (1.1×) and each card off centre is a fixed
  // fraction of the one nearer in, so size falls off fast and the current week
  // clearly dominates (1.1 → 0.75 → 0.51 → 0.35 …).
  function cardScale(ao) { return Math.max(0.22, 1.1 * Math.pow(0.68, ao)); }
  function applyCard(el, o, isCenter, dateStr) {
    ...
    if (ao > CF_WINDOW + 0.5) {
      ...
      el.style.transform = "translateX(" + (s * 250) + "px) rotateY(" + (-s * 58) + "deg) scale(" + cardScale(ao) + ")";
