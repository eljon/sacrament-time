...
(function () {
  ...
  function renderAll() {
    ...
    refreshDots();   // repaint card bars once here (records changed), not per drag frame
  }
  ...
  // A steady ratchet LOOP — the clock hands whirring — under the loading spin. It
  // loops for as long as the save/load is in flight and stops (with a quick fade)
  // when it completes. Synthesised into a short seamless buffer (no asset), all on
  // the audio thread. Only sounds when Web Audio is already running: browsers block
  // audio on a fresh page load until a user gesture, so a cold load is silent, but
  // it plays on the save spin (after the record tap) and any load with audio live.
  var _spinSrc = null, _spinBuf = null;
  function makeSpinBuffer(ctx) {
    var sr = ctx.sampleRate, dur = 0.21, N = Math.floor(sr * dur), buf = ctx.createBuffer(1, N, sr), d = buf.getChannelData(0);
    for (var t = 0; t < dur - 0.02; t += 0.042) {          // 5 clicks × 0.042s = 0.21s → seamless rhythm across the loop
      var start = Math.floor(t * sr), len = Math.floor(0.012 * sr);
      for (var i = 0; i < len && start + i < N; i++) {
        var tt = i / sr;
        d[start + i] += Math.sin(2 * Math.PI * 2000 * tt) * Math.exp(-tt * 220) * 0.5;
      }
    }
    return buf;
  }
  function playClockSpin() {
    var ctx = _actx;
    if (!ctx || ctx.state !== "running") return;
    stopClockSpin();
    try {
      if (!_spinBuf) _spinBuf = makeSpinBuffer(ctx);
      var g = ctx.createGain(); g.gain.value = 0.4; g.connect(ctx.destination);
      var s = ctx.createBufferSource(); s.buffer = _spinBuf; s.loop = true; s.connect(g); s.start(0);
      _spinSrc = { s: s, g: g };
    } catch (e) {}
  }
  function stopClockSpin() {
    var sp = _spinSrc; if (!sp) return;
    _spinSrc = null;
    try {
      var ctx = _actx, t = ctx ? ctx.currentTime : 0;
      sp.g.gain.setValueAtTime(sp.g.gain.value, t);
      sp.g.gain.exponentialRampToValueAtTime(0.0001, t + 0.09);   // quick fade so the stop isn't a click
      sp.s.stop(t + 0.11);
    } catch (e) { try { sp.s.stop(); } catch (e2) {} }
  }
  ...
  function applyCard(el, o, isCenter, dateStr) {
    ...
    if (ao > CF_WINDOW + 0.5) {
      // Off-window cards all share the SAME clamped pose (scale + x both clamp), so
      // re-writing their styles every frame is pure waste — park each ONCE when it
      // first leaves the window, then skip it. This is the bulk of the per-frame
      // cost on a big deck (~2 years of Sundays); skipping it is what keeps the
      // drag light enough that the tick sound doesn't tip it into jank.
      if (el._parked) return;
      el._parked = true;
      // iOS Safari doesn't clip 3D-transformed children with overflow:hidden, so
      // park them at the window edge (not off-screen) or the page pans sideways.
      ...
      el.classList.remove("is-center"); return;
    }
    el._parked = false;
    ...
    // NB: setDot is NOT called here — a card's bar colour depends only on its
    // (static) record, so it's set once in refreshDots when records change, not
    // re-queried for all ~105 cards on every drag frame.
  }
  // Paint every card's status bar from its record. Called when records load/change,
  // never during a drag.
  function refreshDots() {
    if (!state.sundays) return;
    for (var i = 0; i < state.sundays.length; i++) {
      var d = state.sundays[i], el = state.cardEls[d];
      if (el) setDot(el, d);
    }
  }
  ...
  function cfTick() {
    ...
    if (cf.target != null) {
      ...
      if (Math.abs(cf.target - cf.pos) < 0.003) { cf.pos = cf.target; cf.target = null; cf.vel = 0; cfLayout(); cfSyncCenter(); cf.raf = null; flushHero(); tick(); renderHistoryGraph(); return; }
    } else {
  ...
  function cfSyncCenter() {
    ...
    state.cfCenter = ci;
    // Once Web Audio is running, buffer ticks are smooth even through a fling, so
    // tick on EVERY crossing (the full experience). Web Audio only starts running
    // after a real tap (e.g. + or edit) on iOS; until then we're on the <audio>
    // fallback that stutters, so stay conservative — tick only while the finger is
    // actively dragging, and once when a fling lands (see cfTick settle).
    if ((_actx && _actx.state === "running") || state.cf.dragging) haptic();
    state.selected = state.sundays[ci];
    state.armed = false; state.heroMode = "auto"; // moving to a new week resets edit state
    // While the FINGER is dragging, update the hero live so the timeline tracks the
    // centred week as you scroll. During a momentum FLING the crossings come too
    // fast to read and rebuilding the whole hero each one hitches, so defer that and
    // flush ONCE on settle.
    if (state.cf.dragging) renderHeroBody();
    else if (state.cf.raf != null) state.heroDirty = true;
    else renderHeroBody();
  }
  function flushHero() { if (state.heroDirty) { state.heroDirty = false; renderHeroBody(); } }
  ...
  function cfUp() {
    ...
    if (reduce) { cf.pos = clampPos(Math.round(cf.pos)); cfLayout(); cfSyncCenter(); flushHero(); tick(); return; }
  ...
  var _actx = null, _buf = null, _keepAlive = null, _pool = [], _pi = 0, _warmed = false, _lastTickT = 0;
  ...
  function audioCtx() {
    ...
    if (!AC) return null;
    try {
      _actx = new AC(); setAudioSession();
      _actx.decodeAudioData(b64ToArrayBuffer(TICK_WAV), function (buf) { _buf = buf; }, function () {});
    } catch (e) { _actx = null; }
    return _actx;
  }
  function keepAlive(ctx) {                 // 0-gain source so iOS won't re-suspend between ticks
    if (_keepAlive || !ctx) return;
    try { var o = ctx.createOscillator(), g = ctx.createGain(); g.gain.value = 0; o.connect(g); g.connect(ctx.destination); o.start(0); _keepAlive = o; } catch (e) {}
  }
  function initTickPool() {
    setAudioSession();
    audioCtx();                             // create context + decode the tick into a buffer
    for (var i = 0; i < 3; i++) {           // tiny <audio> fallback (only used until Web Audio runs)
      var a = new Audio(TICK_WAV); a.preload = "auto"; try { a.load(); } catch (e) {}
      _pool.push(a);
    }
  }
  // First gesture: resume Web Audio AND play one <audio> to activate the iOS audio
  // session. An <audio> play activates from a DRAG (which resume() alone won't) and
  // that lets Web Audio run — after which ticks are cheap buffer plays OFF the main
  // thread, with no <audio> per tick, so no freeze.
  function unlockTick() {
    setAudioSession();
    var ctx = audioCtx();
    ...
    _warmed = true;
    var a = _pool[0];
    if (a) { try { a.muted = false; var p = a.play(); if (p && p.catch) p.catch(function () {}); } catch (e) {} }
  }
  function tick() {
    var now = (window.performance && performance.now) ? performance.now() : Date.now();
    if (now - _lastTickT < 35) return;                 // mild pileup guard
    _lastTickT = now;
    var ctx = _actx;
    if (ctx && ctx.state === "running" && _buf) {      // Web Audio buffer — off-thread, effectively free
      try {
        var s = ctx.createBufferSource(); s.buffer = _buf;
        var g = ctx.createGain(); g.gain.value = 0.5;   // per-source gain, torn down with the source (clean graph)
        s.connect(g); g.connect(ctx.destination); s.start(0);
      } catch (e) {}
      return;
    }
    if (ctx && ctx.state !== "running") { try { ctx.resume(); } catch (e) {} }   // keep nudging it toward running
    if (!_pool.length) return;                          // <audio> fallback (drag-capable) until Web Audio runs
    var a = _pool[_pi]; _pi = (_pi + 1) % _pool.length;
    try { a.muted = false; var p2 = a.play(); if (p2 && p2.catch) p2.catch(function () {}); } catch (e) {}
  }
  ...
  function haptic() {
    ...
    // NB: the <input switch> Taptic trick is NOT toggled per crossing — it does
    // nothing on iOS 17.4+ (patched) and, being position:fixed, toggling it mid-
    // drag forced iOS compositor churn (a hitch). The element is kept (warmed once
    // on first touch) but no longer clicked on every snap.
    tick();
  }
  function init() {
    initHaptics();
