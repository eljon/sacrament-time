...
(function () {
  ...
  var _actx = null, _lastTick = -1;
  function audioCtx() {
    if (_actx) return _actx;
    var AC = window.AudioContext || window.webkitAudioContext;
    if (!AC) return null;
    try { _actx = new AC(); } catch (e) { _actx = null; }
    return _actx;
  }
  function tick() {
  ...
  function init() {
    initHaptics();
    // prime the Taptic engine AND unlock the audio context on the very first touch
    // (strong user activation) — but silently, so a stray first tap doesn't tick.
    window.addEventListener("pointerdown", function primeFeedback() {
      var ctx = audioCtx();
