...
(function () {
  ...
  // A decelerating ratchet — the sound of the clock hands winding to a stop — under
  // the loading spin. Synthesised (no asset) and scheduled entirely on the audio
  // thread. Only sounds when Web Audio is already running: browsers block audio on
  // page load until a user gesture, so on a fresh load this is silent, but it plays
  // on the save spin (after the record tap) and any load where audio is unlocked.
  function playClockSpin() {
    var ctx = _actx;
    if (!ctx || ctx.state !== "running") return;
    var t = ctx.currentTime, elapsed = 0, interval = 0.02;
    var out = ctx.createGain(); out.gain.value = 0.45; out.connect(ctx.destination);
    while (elapsed < 0.85) {
      var when = t + elapsed;
      ...
      g.gain.exponentialRampToValueAtTime(0.0001, when + 0.012);   // short ratchet click
      o.connect(g); g.connect(out);
      o.start(when); o.stop(when + 0.016);
      elapsed += interval; interval *= 1.07;                        // hands winding down
    }
  }
