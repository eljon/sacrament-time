...
(function () {
  ...
  // Bar-per-week chart: height encodes how late the week ran, colour reuses the
  // start→end gradient. On first reveal the bars fly down, rotate upright and
  // grow — echoing the calendar bars dropping in to form the graph.
  function renderHistoryGraph() {
    var section = $("historyGraph"), plot = $("hgPlot");
    var recs = sortedRecords().slice(-12);
    ...
    if (sig === state.hgSig) return;            // data unchanged → keep current bars & animation state
    state.hgSig = sig;

    var MINH = 14, MAXH = 90, CAP = 12;         // px, px, minutes-late that maxes the bar
    plot.innerHTML = "";
    recs.forEach(function (r, i) {
      var a = analyze(r);
      var late = Math.max(0, a.startDev || 0) + Math.max(0, a.endDev || 0);
      var h = MINH + Math.min(1, late / CAP) * (MAXH - MINH);
      var cs = lateColor(a.startDev), ce = lateColor(a.endDev);
      var d = parseYMD(r.date);
      var col = document.createElement("div");
      col.className = "hg-col" + (r.date === state.selected ? " is-current" : "");
      col.innerHTML =
        '<div class="hg-bar-wrap"><span class="hg-bar" style="height:' + h.toFixed(1) +
        'px;background:linear-gradient(to top,' + cs + ',' + ce + ')"></span></div>' +
        '<div class="hg-date"><span class="hg-d">' + d.getDate() + '</span><span class="hg-m">' + MON[d.getMonth()].toUpperCase() + '</span></div>';
      var delay = (i * 55) + "ms";
      col.querySelector(".hg-bar").style.transitionDelay = delay;
      col.querySelector(".hg-date").style.transitionDelay = delay;
      col.title = shortDate(r.date) + " · " + (late === 0 ? "on time" : late + " min late");
      col.onclick = function () { var idx = state.sundays ? state.sundays.indexOf(r.date) : -1; if (idx >= 0) animateTo(idx); scrollToHero(); };
      plot.appendChild(col);
    });

    if (state.hgShown || prefersReduced()) { section.classList.add("instant", "in"); }
    else { section.classList.remove("in", "instant"); maybeRevealGraph(); }
  }
  // Add .in (playing the fly-in) the first time the graph scrolls into view.
  function maybeRevealGraph() {
    var section = $("historyGraph");
    if (!section || section.hidden || state.hgShown) return;
    var r = section.getBoundingClientRect();
    var vh = window.innerHeight || document.documentElement.clientHeight;
    if (r.top < vh * 0.88 && r.bottom > 0) {
      requestAnimationFrame(function () { section.classList.add("in"); state.hgShown = true; });
    }
  }
  ...
  function init() {
    ...
    // reveal the history graph's fly-in when it scrolls into view
    var revealTick = false;
    window.addEventListener("scroll", function () {
      if (revealTick) return; revealTick = true;
      requestAnimationFrame(function () { revealTick = false; maybeRevealGraph(); });
    }, { passive: true });
