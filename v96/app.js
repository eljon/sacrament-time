...
(function () {
  ...
  function setupFly() {
    ...
    state.hgItems.forEach(function (it) {
      ...
      var dayEl = flyClone(calDay, layer, it.date === state.selected ? { color: "var(--brand)" } : null);
  ...
  function updateFly() {
    ...
    f.groups.forEach(function (g) {
      ...
      // display:none when idle so these clones can never sit off-screen; only
      // written when the active state changes, so there's no per-frame churn.
      if (g._active !== active) {
        g._active = active;
        var d = active ? "block" : "none";
        g.day.el.style.display = d; g.mon.el.style.display = d; g.bar.el.style.display = d;
      ...
      // position tracks the scroll LINEARLY (p) so the clone stays in view and
      // glides down with the page; only the look (rotate/scale/roll) eases (eP).
      flyPiece(f, g, g.day, src, ry, z, eP, p, null);
      flyPiece(f, g, g.mon, src, ry, z, eP, p, null);
      flyPiece(f, g, g.bar, src, ry, z, eP, p, eP);   // bar also rolls 90° + stretches
    });
  }
  function flyPiece(f, g, pc, src, ry, z, eP, lp, barMorph) {
    var sc = src.scale + (pc.dstScale - src.scale) * eP;
    // "inner" = the card's EXACT projection at this progress, at the card's own fan
    // position (rotateY/translateZ/scale decaying). The perspective wraps ONLY this
    // local transform, so it never distorts as the piece flies. The graph-ward
    // travel is a flat screen-space translate applied on top (below).
    var inner = new DOMMatrix().translateSelf(f.baseX, f.baseY).multiply(f.P)
      .multiply(new DOMMatrix().translateSelf(src.x, 0, z).rotateSelf(0, ry, 0).scaleSelf(sc, sc))
      .translate(pc.offX, pc.offY);
    if (barMorph != null) {
      // in the bar's own box: roll 90° and stretch the calendar bar into the tall
      // vertical graph bar (about its centre) — layered on the exact 3D placement
      var cx = pc.w / 2, cy = pc.h / 2;
      var w = pc.w + (pc.barTW - pc.w) * barMorph, h = pc.h + (pc.barTH - pc.h) * barMorph;
      inner = inner.multiply(new DOMMatrix().translateSelf(cx, cy).rotateSelf(0, 0, 90 * barMorph)
        .scaleSelf(w / pc.w, h / pc.h).translateSelf(-cx, -cy));
    }
    // where the piece centre currently projects to, and where we want it (a linear
    // glide from the exact calendar centre to the graph centre) — the difference is
    // a pure screen-space translate, so size shrinks progressively with no bulge.
    var C = inner.transformPoint(new DOMPoint(pc.w / 2, pc.h / 2, 0, 1));
    var cx0 = C.x / C.w, cy0 = C.y / C.w;
    var wantX = pc.CsrcX + (pc.PtX - pc.CsrcX) * lp, wantY = pc.CsrcY + (pc.PtY - pc.CsrcY) * lp;
    pc.el.style.transform = new DOMMatrix().translateSelf(wantX - cx0, wantY - cy0).multiply(inner).toString();
  }
  ...
  // ---- Wire up --------------------------------------------------------------
  // ---- Haptics --------------------------------------------------------------
  // navigator.vibrate covers Android; iOS Safari ignores it, so we also toggle a
  // Safari <input switch> via its <label>, which fires the system tick on iOS 17.4+.
  // The switch must be RENDERED for the tick to fire — so it's visually-hidden with
  // the clip pattern, NOT display:none / visibility:hidden / opacity:0.
  var _hapticSwitch = null, _hapticLabel = null;
  var HAPTIC_HIDE = "position:fixed;bottom:0;left:0;width:12px;height:12px;margin:0;padding:0;border:0;opacity:0.001;pointer-events:none;transform:scale(0.05);transform-origin:bottom left;";
  function initHaptics() {
    _hapticSwitch = document.createElement("input");
    _hapticSwitch.type = "checkbox"; _hapticSwitch.id = "_hapticSwitch";
    _hapticSwitch.setAttribute("switch", "");           // Safari-only switch control
    _hapticSwitch.style.cssText = HAPTIC_HIDE;
    _hapticLabel = document.createElement("label");
    _hapticLabel.setAttribute("for", "_hapticSwitch");
    _hapticLabel.setAttribute("aria-hidden", "true");
    _hapticLabel.style.cssText = HAPTIC_HIDE + "z-index:-1;";
    document.body.appendChild(_hapticSwitch);
    document.body.appendChild(_hapticLabel);
  }
  function haptic() {
    if (navigator.vibrate) { try { navigator.vibrate(6); } catch (e) {} }
    // toggle the switch via its label — this is what pulses the Taptic Engine on iOS
