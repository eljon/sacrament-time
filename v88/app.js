...
(function () {
  ...
  function setupFly() {
    ...
    state.fly = { groups: groups, anchored: false, start: 0, endDown: 1, endUp: 1, assembled: false };
    refreshFlyAnchors();
    updateFly();
  }
  function docBox(el) {
  ...
  function updateFly() {
    ...
    // use the later end-point once assembled so the fly stays landed longer on the
    // way back (starts flying apart at ~55% instead of the ~33% it landed at)
    var end = f.assembled ? f.endUp : f.endDown;
    var p = (sy - f.start) / (end - f.start);
    if (f.maxScroll != null && sy >= f.maxScroll - 1) p = 1;   // can't scroll further → fully assemble
    p = p < 0 ? 0 : p > 1 ? 1 : p;
    if (p >= 1) f.assembled = true; else if (p <= 0) f.assembled = false;
