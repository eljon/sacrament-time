...
(function () {
  ...
  function cfTick() {
    ...
    if (cf.target != null) {
      ...
      if (Math.abs(cf.target - cf.pos) < 0.003) { cf.pos = cf.target; cf.target = null; cf.vel = 0; cfLayout(); cfSyncCenter(); cf.raf = null; flushHero(); renderHistoryGraph(); return; }
    } else {
  ...
  function cfUp() {
    ...
    if (reduce) { cf.pos = clampPos(Math.round(cf.pos)); cfLayout(); cfSyncCenter(); flushHero(); return; }
  ...
  var _actx = null, _buf = null, _master = null, _keepAlive = null, _pool = [], _pi = 0, _warmed = false, _lastTickT = 0;
  ...
  function audioCtx() {
    ...
    try {
      ...
      _master = _actx.createGain(); _master.gain.value = 0.5; _master.connect(_actx.destination);   // reused every tick → zero per-tick node alloc
    } catch (e) { _actx = null; }
    return _actx;
  }
  ...
  function tick() {
    ...
    _lastTickT = now;
    var ctx = _actx;
    if (ctx && ctx.state === "running" && _buf && _master) {   // Web Audio buffer — off-thread, effectively free
      try {
        var s = ctx.createBufferSource(); s.buffer = _buf;
        s.connect(_master); s.start(0);                        // reuse the master gain — only the one-shot source is allocated
      } catch (e) {}
      return;
    }
