...
(function () {
  ...
  // Progress is tied directly to scroll position (1:1, no lag) — the smoothness
  // comes from the transform/opacity-only render below, not from easing the
  // scroll. rAF-throttled by the scroll listener so it runs at most once a frame.
  function updateFly() {
    var f = state.fly; if (!f) return;
