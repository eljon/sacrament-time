...
(function () {
  ...
  function setupFly() {
    ...
    state.hgItems.forEach(function (it) {
      ...
      var pc = function (el, kind) {
        return { el: el, kind: kind, w: el.offsetWidth || 1, h: el.offsetHeight || 1,
  ...
  function refreshFlyAnchors() {
    ...
    f.groups.forEach(function (g) {
      ...
      // graph landing centre + target scale for each piece
      prepPiece(g, g.day, tDay, g.day.natH ? tDay.h / g.day.natH : 1);
      prepPiece(g, g.mon, tMon, g.mon.natH ? tMon.h / g.mon.natH : 1);
      prepPiece(g, g.bar, tBar, 1);
      g.bar.barTW = tBar.h;   // graph bar length (becomes vertical after the 90° roll)
      g.bar.barTH = tBar.w;   // graph bar thickness
    });
    f.anchored = true;
  }
  // Cache a piece's geometry: its box offset from the card centre, and the flat
  // landing translate (tx1/ty1) that puts its centre exactly on the graph target.
  function prepPiece(g, pc, target, dstScale) {
    pc.offX = pc.offL - g.cardHW; pc.offY = pc.offT - g.cardHH;        // box top-left vs card centre
    pc.cRelX = pc.offX + pc.w / 2; pc.cRelY = pc.offY + pc.h / 2;       // box centre vs card centre
    pc.dstScale = dstScale;
    var f = state.fly;
    pc.tx1 = target.cx - f.baseX - dstScale * pc.cRelX;
    pc.ty1 = target.cy - f.baseY - dstScale * pc.cRelY;
  }
  ...
  function flyPiece(f, g, pc, src, ry, z, eP, lp, barMorph) {
    ...
    var tx = src.x + (pc.tx1 - src.x) * lp, ty = pc.ty1 * lp;
    // M = Translate(base) · Perspective · Card(tx,z,ry,scale) · Translate(pieceOffset)
    var M = new DOMMatrix().translateSelf(f.baseX, f.baseY).multiply(f.P)
      .multiply(new DOMMatrix().translateSelf(tx, ty, z).rotateSelf(0, ry, 0).scaleSelf(sc, sc))
    ...
    if (barMorph != null) {
      ...
      M = M.multiply(new DOMMatrix().translateSelf(cx, cy).rotateSelf(0, 0, 90 * barMorph)
        .scaleSelf(w / pc.w, h / pc.h).translateSelf(-cx, -cy));
    }
    pc.el.style.transform = M.toString();
  }
