...
(function () {
  ...
  function cfSyncCenter() {
    ...
    state.cfCenter = ci;
    // Tick per notch ONLY while the finger is dragging (deliberate, paced). During
    // a momentum fling, crossings come in fast bursts and the per-crossing audio is
    // what stutters — and you can't hear individual ticks at that speed anyway — so
    // stay silent through the fling and tick once when it lands (see cfTick settle).
    if (state.cf.dragging) haptic();
